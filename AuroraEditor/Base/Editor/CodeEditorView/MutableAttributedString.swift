...
extension NSMutableAttributedString {
    ...
    ///
    /// The tokeniser is based on an eager regular expression matcher. Hence, it will match the first matching \
    /// alternative in a sequence of alternatives. To deal with string patterns, where some patterns may be a prefix \
    /// of another, the string patterns are turned into regular expression alternatives longest string first. \
    /// However, pattern consisting of regular expressions are tried in an indeterminate order.\
    /// Hence, no pattern should have as a full match a prefix of another pattern's full match,\
    /// to avoid indeterminate results. Moreover, strings match before patterns that cover the same lexeme.
    ///
    /// For each token that has got a multi-character lexeme,\
    /// the tokeniser attributes the first character of that lexeme with a token attribute marked \
    /// as being the lexeme head character.\ All other characters of the lexeme —\
    ...
    public static func tokeniser<TokenType, StateType: TokeniserState>(
        ...
        throws -> Tokeniser<TokenType, StateType>.State {
            ...
            // NB: Be careful with the re-ordering, because the order in `patternTokenTypes`
            // below must match the order of the patterns in the alternatives of the regular
            ...
            let orderedMap = stateMap.sorted { (lhs, rhs) in return lhs.key > rhs.key },
                pattern    = orderedMap.reduce("") { (regexp, mapEntry) in
                    ...
                    switch mapEntry.key {
                    case .string(let lexeme):
                        regexpPattern = NSRegularExpression.escapedPattern(for: lexeme)
                    case .word(let lexeme):
                        regexpPattern = "\\b" + NSRegularExpression.escapedPattern(for: lexeme) + "\\b"
                    case .pattern(let pattern):
                        regexpPattern = "(" + pattern + ")" // each pattern gets a capture group
                    }
                    if regexp.isEmpty { return regexpPattern } else { return regexp + "|" + regexpPattern}
                }
            let stringTokenTypes: [
    ...
    public func tokeniseAndSetTokenAttribute<TokenType, StateType>(attribute: NSAttributedString.Key,
        ...
        while currentRange.length > 0 {
            ...
            for resultNumber in stride(from: result.numberOfRanges - 1, through: 1, by: -1)
            where result.range(at: resultNumber).location != NSNotFound {
                tokenAction = stateTokeniser.patternTokenTypes[resultNumber - 1]
            }

            // If it wasn't a matching group, it must be a simple string match
            if tokenAction == nil {
                // no capture group matched => we matched a simple string lexeme
                tokenAction = stateTokeniser.stringTokenTypes[
                    (self.string as NSString).substring(with: result.range)
                ]
            }
            ...
            if let action = tokenAction, result.range.length > 0 {
                ...
                if result.range.length > 1 {
                    self.addAttribute(attribute,
                                      ...
                                      range: NSRange(
                                        location: result.range.location + 1,
